...
import static com.bmqb.patternlockview.PatternLockView.AspectRatio.ASPECT_RATIO_HEIGHT_BIAS;
import static com.bmqb.patternlockview.PatternLockView.AspectRatio.ASPECT_RATIO_SQUARE;
import static com.bmqb.patternlockview.PatternLockView.AspectRatio.ASPECT_RATIO_WIDTH_BIAS;
import static com.bmqb.patternlockview.PatternLockView.PatternViewMode.AUTO_DRAW;
import static com.bmqb.patternlockview.PatternLockView.PatternViewMode.CORRECT;
import static com.bmqb.patternlockview.PatternLockView.PatternViewMode.WRONG;
...
public class PatternLockView extends View {
    ...
    public PatternLockView(Context context, AttributeSet attrs) {
        ...
        try {
            sDotCount =
                typedArray.getInt(R.styleable.PatternLockView_dotCount, DEFAULT_PATTERN_DOT_COUNT);
            mAspectRatioEnabled =
                typedArray.getBoolean(R.styleable.PatternLockView_aspectRatioEnabled, false);
            mAspectRatio =
            ...
            mDotNormalSize =
                (int) typedArray.getDimension(R.styleable.PatternLockView_dotNormalSize,
                    ResourceUtils.getDimensionInPx(getContext(), R.dimen.pattern_lock_dot_size));
            mDotSelectedSize =
                (int) typedArray.getDimension(R.styleable.PatternLockView_dotSelectedSize,
                    ResourceUtils.getDimensionInPx(getContext(),
                        R.dimen.pattern_lock_dot_selected_size));
            mDotAnimationDuration =
                typedArray.getInt(R.styleable.PatternLockView_dotAnimationDuration,
                    DEFAULT_DOT_ANIMATION_DURATION);
            mPathEndAnimationDuration =
                typedArray.getInt(R.styleable.PatternLockView_pathEndAnimationDuration,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mPatternViewMode == AUTO_DRAW) {
            ...
            boolean needToUpdateInProgressPoint = numCircles > 0 && numCircles < patternSize;
            ...
            if (needToUpdateInProgressPoint) {
                ...
                float dx =
                    percentageOfNextCircle * (getCenterXForColumn(nextDot.mColumn) - centerX);
                float dy = percentageOfNextCircle * (getCenterYForRow(nextDot.mRow) - centerY);
        ...
        for (int i = 0; i < sDotCount; i++) {
            ...
            for (int j = 0; j < sDotCount; j++) {
                ...
                drawCircle(canvas, (int) centerX, (int) centerY + translationY, size,
                    drawLookupTable[i][j], dotState.mAlpha);
            }
        }
        ...
        if (drawPath) {
            ...
            if ((mPatternInProgress || mPatternViewMode == AUTO_DRAW) && anyCircles) {
                ...
                mPathPaint.setAlpha(
                    (int) (calculateLastSegmentAlpha(mInProgressX, mInProgressY, lastX, lastY)
                        * 255f));
                canvas.drawPath(currentPath, mPathPaint);
            }
        }
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        return new SavedState(superState, PatternLockUtils.patternToString(this, mPattern),
            mPatternViewMode, mInputEnabled, mInStealthMode, mEnableHapticFeedback);
    }
     ...
     *
     * @param patternViewMode
     *     The mode in which the pattern should be displayed
     * @param pattern
     *     The pattern
     */
    ...
    private void startLineEndAnimation(final DotState state, final float startX, final float startY,
    ...
    private void handleActionMove(MotionEvent event) {
        ...
        for (int i = 0; i < historySize + 1; i++) {
            ...
            if (mPatternInProgress && patternSize > 0) {
                ...
                // location
                mTempInvalidateRect.union(Math.round(left), Math.round(top), Math.round(right),
                    Math.round(bottom));
            }
        }
    ...
    public static class Dot implements Parcelable {
        ...
        private static void checkRange(int row, int column) {
            if (row < 0 || row > sDotCount - 1) {
                throw new IllegalArgumentException("mRow must be in range 0-" + (sDotCount - 1));
            }
            if (column < 0 || column > sDotCount - 1) {
                throw new IllegalArgumentException("mColumn must be in range 0-" + (sDotCount - 1));
            }
        }
        ...
        public boolean equals(Object object) {
            if (object instanceof Dot) {
                return mColumn == ((Dot) object).mColumn && mRow == ((Dot) object).mRow;
            }
